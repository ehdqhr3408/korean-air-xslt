<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/"> 
    <!-- set request message -->	
    <xsl:variable name="reqDataby">
    	<dp:serialize select="dp:variable('var://context/MYVAR/reqData')" disable-output-escaping="yes" omit-xml-decl="yes"/>
    </xsl:variable>
    <xsl:variable name="reqData">
    	<xsl:value-of select="$reqDataby" disable-output-escaping="yes"/>
    </xsl:variable>    
    <!-- set response message -->
    <xsl:variable name="resDataby">
    	<dp:serialize select="dp:variable('var://context/MYVAR/resData')" omit-xml-decl="yes"/>
    </xsl:variable>
    <xsl:variable name="resData">
    	<xsl:value-of select="$resDataby" disable-output-escaping="yes" />
    </xsl:variable>
      ...
      <!--LOG--><xsl:value-of select="func:LoggingInfo('Error Lgging Start',$hopCount)"/>		
    	<xsl:variable name="ImsLogLevel" select="string('ERROR')"/>

      <xsl:variable name="HBinfo" select="func:HBinfo($hopCount,$ImsLogLevel)"/>

      <xsl:if test=" $HBinfo != 'N_N' ">
      ...
      <xsl:variable name="guid"><xsl:value-of select="substring($guid,1,33)" /></xsl:variable>
      ...
      <!--LOG--><xsl:value-of select="func:LoggingDebug('hopCount ============================',$hopCount)"/>
      ...
      <xsl:variable name="pipServiceName"><xsl:value-of select="dp:variable('var://context/MYVAR/IFID')" /></xsl:variable>
      ...
      <!--LOG--><xsl:value-of select="func:LoggingInfo('XresponseSystem  ',$XresponseSystem)"/>
        ...
        <xsl:value-of select="str:padding(5-number(string-length(dp:variable('var://context/MYVAR/procdtsndsyscd'))),' ')" />    	
				<xsl:choose>
					<xsl:when test=" contains ($endpointurl,'PIP') ">
...
<!-- ims logging list add end 20130816 -->  
      <xsl:variable name="sid"><xsl:value-of select="dp:variable('var://context/MYVAR/ServerID')" /></xsl:variable>
      ...
      <xsl:variable name="rtncd">2</xsl:variable>

      <xsl:variable name="ErrorCode"><xsl:value-of select="dp:variable('var://context/MYVAR/errorCode')" /></xsl:variable>
      <xsl:variable name="ErrorCode"><xsl:value-of select="substring($ErrorCode,1,10)" /><xsl:value-of select="str:padding(10-number(string-length($ErrorCode)),' ')" /></xsl:variable>

      <xsl:variable name="ErrorContents"><xsl:value-of select="dp:variable('var://context/MYVAR/errorMsg')" /></xsl:variable>
      <xsl:variable name="ErrorContents"><xsl:value-of select="substring($ErrorContents,1,100)" /><xsl:value-of select="str:padding(100-number(string-length($ErrorContents)),' ')" /></xsl:variable>

      <xsl:variable name="ErrorReason">
				<xsl:value-of select="func:MakeErrorReason()"/>
      </xsl:variable>
      ...
      <!--LOG--><xsl:value-of select="func:LoggingInfo('ErrorReason ============================',$ErrorReason)"/>
			<xsl:variable name="Message" select="func:ImsLogging(func:HBinfo($hopCount,$ImsLogLevel),'N',$requestData,$requestHeader,$requestBody)"/>
			<!--LOG--><xsl:value-of select="func:LoggingInfo('result Message msg ============================',$Message)"/>
			
			<xsl:variable name="reqData" select="func:ImsLogging('Y_Y','N',$reqData)"/>
			<!--LOG--><xsl:value-of select="func:LoggingInfo('result reqData msg ============================',$reqData)"/>
			<xsl:variable name="resData" select="func:ImsLogging('Y_Y','N',$resData)"/>
			<!--LOG--><xsl:value-of select="func:LoggingInfo('result resData msg ============================',$resData)"/>
			...
			<!--LOG--><xsl:value-of select="func:LoggingDebug('result destServiceNamemsg ============================',$destServiceName)"/>
			...
			<!--LOG--><xsl:value-of select="func:LoggingDebug('result reqData msg ============================',$reqData)"/>
			<!--LOG--><xsl:value-of select="func:LoggingDebug('result Message msg ============================',$Message)"/>
      ...
      <xsl:variable name="CRLF"  select="'&#13;&#10;'"/>                
			<xsl:variable name="wholeMessage">
				...
				<xsl:value-of disable-output-escaping="yes" select="concat('Request Message : ',$CRLF)" />
				<xsl:value-of disable-output-escaping="yes" select="$reqData" />
				<xsl:value-of disable-output-escaping="yes" select="concat($CRLF,'Response Message : ',$CRLF)" />				
				<xsl:value-of disable-output-escaping="yes" select="$resData" />
        <xsl:value-of disable-output-escaping="yes" select="concat($CRLF,'Client Return Message : ',$CRLF)"/> 
      ...
      <xsl:variable name="mqmdStr">
	   ...
	   <!--LOG--><xsl:value-of select="func:LoggingDebug('getMQUrl ===================',$targetURL)"/>
        <xsl:variable name="targetURL" select="func:getMQUrl()"/>
        <xsl:value-of select="func:LoggingDebug('result targetURL ',$targetURL)"/>
     <!--LOG--><xsl:value-of select="func:LoggingDebug('wholeMessage ===================',$wholeMessage)"/> 
      ...
      <dp:set-variable name="'var://context/MYVAR/urlOpenResult'" value="$urlOpenResult" />
      
      </xsl:if>
   <!--LOG--><xsl:value-of select="func:LoggingInfo('Lgging End',$hopCount)"/>
      <xsl:copy-of select="." />
