<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">   
		  ...
		  <xsl:variable name="isServer">
		  	<xsl:value-of select="contains(dp:variable('var://service/formatted-error-message'), ':Server')"/>
		  </xsl:variable>
			<xsl:choose>
		  ...
		  <xsl:variable name="workstationId"><xsl:value-of select="dp:variable('var://context/MYVAR/workstationId')"/></xsl:variable>
			...
			<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:pip="http://PIP_Service_Library/PIP_Processing" xmlns:pips="http://PIP_Service_Library/PIP_Session">
		      ...
		      <pips:Session>
		         ...
		         <pips:sessionId><xsl:value-of select="/*[local-name()='Envelope']/*[local-name()='Header']/*[local-name()='Session']/*[local-name()='sessionId']/text()" /></pips:sessionId>
		         <pips:sequenceNumber><xsl:value-of select="/*[local-name()='Envelope']/*[local-name()='Header']/*[local-name()='Session']/*[local-name()='sequenceNumber']/text()" /></pips:sequenceNumber>
		         <pips:securityToken><xsl:value-of select="/*[local-name()='Envelope']/*[local-name()='Header']/*[local-name()='Session']/*[local-name()='securityToken']/text()" /></pips:securityToken>
		      </pips:Session>
 				</soapenv:Header>
				<soapenv:Body>
					<soapenv:Fault>
