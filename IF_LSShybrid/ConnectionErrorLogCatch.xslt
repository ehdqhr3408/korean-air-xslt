<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">   
    	<!--1220 add-->
			<dp:set-variable name="'var://context/MYVAR/faultactor'" value="string('IM/SSO')"/>

			<xsl:variable name="codeId">IMS.S01</xsl:variable>

	 <xsl:variable name="LOGGING" select="func:LoggingDebug('get WdpErrorCode start',$codeId)"/>
    <xsl:variable name="ErrorCodeResult" select="func:getWdpErrorCode('PIP_ERROR_01',$codeId)"/>
 <xsl:variable name="LOGGING" select="func:LoggingDebug('get WdpErrorCode',$ErrorCodeResult)"/>
